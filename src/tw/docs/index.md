...
絕大多數輸入法，簡體字和對應的繁體字都佔用相同的碼位，導致輸入繁體時需要選重。比如五筆字型中，`簡` `简`兩個字的編碼都是`TUJf`。如果我想打繁體的「簡」，需要按一下選重鍵。因此，大多數輸入法在繁體文本下，不具備盲打的優勢，除非重新對碼錶進行排序。

宇浩輸入法繁簡漢字分離，不存在共用碼位現象，故而使用一張碼錶即可既打簡又打繁，不用擔心選重問題。你可以按照自己掌握每一個字的繁簡狀態，不需要軟件轉換，更不需要準備兩套碼錶。可以像寫字一樣隨心所欲。
...
宇浩輸入法，單字輸入模式下（只考慮二簡）選重率約為0.06%，詞語輸入模式下[選重率約為1.27%](./statistics.md)。一個比較好的狀態，應該是主要打單字，輔助打詞語，根據個人偏好，找到一個平衡點，讓綜合選重率達到0.30% 到 0.50% 左右。

因此，宇浩輸入法的最佳輸入體驗，是以字為主，以詞為輔，激進打字，謹慎打詞，從而達到較佳的確定性，做到完全關閉候選框輸入。
