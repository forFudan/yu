...
本文只是呈現輸入法的客觀測評數據，不能用來作為此輸入方案是否優秀的唯一評價標準。評判一個輸入方案，應該綜合考量以下因素：
...
| 方案         | 簡體選重率 | 繁體選重率 | 繁簡混合選重率 | GB2312 | 國字常用 |   GBK |
| :----------- | ---------: | ---------: | -------------: | -----: | -------: | ----: |
| **四碼定長** |            |            |                |        |          |       |
...
情況一：如果詞庫為**中等大小**，換言之，分詞比較適中，我們對這句話進行如下分割：
...
情況二：如果詞庫為**超大詞庫**，換言之，分詞比較激進，我們對這句話進行如下分割：
...
情況三：如果詞庫為**精簡詞庫**，換言之，分詞比較保守，我們對這句話進行如下分割：
...
- 用戶必定打長度較長的詞。如：出現「電視機」時，必定打「電視機」，而不是「電視·機」或「電·視·機」。這一條也意味着，一個多字詞若在字頻表中，那麼用戶絕對不會打成單字。

因為這兩個條件過於完美，實際使用時對用戶的分詞壓力高，所以一般不會實現。很多時候，用戶會將某些詞頻表中的多字詞分為單字詞輸入，這意味着：
