...
符合筆順，指的是字根之間不穿插寫成[^order]。它有兩個要求：

- 整體符合筆順：指一個字完全按照筆順寫成，字根間不會互相干擾筆順。`固`字如拆`冂古一`，便算作整體符合筆順。
- 局部符合筆順：指一個字雖不能完全按照筆順寫成，但局部字根符合筆順。`固`字如拆`囗古`，便算作局部符合筆順，且符合筆順的有一個字根「古」，共五畫。

我們首先判斷一個字是否整體符合筆順，如果不是，再判斷局部符合筆順的大小。局部符合筆順的字根數量更多、筆劃更多者勝出。^[這是爲了防止以下情況，一個字 A 有 X Y 兩部分構成，X 部分拆 X1 X2，根少但不符合筆順。Y 部分，可以拆 Y1 Y2，符合筆順，且字根相交；或拆 Y3 Y4，不符合筆順，但字根分散。如果必須滿足**整體符合筆順**，則 Y 拆 Y1 Y2 雖然符合筆順，但由於 X 不符合筆順，A 整體依舊不符合筆順。那麽 Y 應該拆成 Y3 Y4，因爲**能散不交**。這樣一來，Y 作爲獨體字和 Y 作爲 A 的一部分時，拆分不一致，我們需要避免這種情況。這就是檢查**局部符合筆順**的原因。]
...
4. 又一例：`曱` yuē `甴` yóu。
   - `曱`按笔顺取大，故而是`日丨`。
   - `甴`，候选拆分为`日丨` `囗丄`。两者都是两根，都結構完整。因爲先寫豎，再寫中間的一横，为了**局部符合筆順**，讓不被穿插的字根盡可能地大，我们取`丄`。故而`囗丄`勝出。
   <div class="flex justify-left flex-wrap">
