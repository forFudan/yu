...
符合笔顺，指的是字根之间不穿插写成[^order]。它有两种情况：
...
另外一种拆法`丌䒑丿`不取的原因是`䒑`字根中的点应当和横相连（艹字头的行书写法），而`严`字中两点分散了。这违反了[字根的内在属性](./intrinsic)。
<div class="flex justify-left flex-wrap">
...
::: tip 例
`宺`拆`宀口一𫶧`而不拆`宀口丌乚`，因为拆为`丌乚`破坏了字根`丌`的对称性。
:::
...
4. 又一例：`曱` yuē `甴` yóu。
   - `曱`按笔顺取大，故而是`日丨`。
   - `甴`，候选拆分为`日丨` `囗丄`。两者都是两根，都结构完整。按笔顺取大，故而是`日丨`。
   <div class="flex justify-left flex-wrap">
